...
export class QvForm {
  ...
  private _triggerValidationEvents = ["change", "qv.form.updated"];
  /**
   * The html form
   */
   ...
   */
  private _initQvInputs(qvInputs?: HTMLElement[]) {
    this._qvInputs = [];
    qvInputs = !!qvInputs
      ? qvInputs
      : Array.from(
  ...
  onUpdate(fn: QvFormHandler) {
    this.on("qv.form.updated", (event: any) => {
      this.__call(fn, event.detail);
    });
  }
